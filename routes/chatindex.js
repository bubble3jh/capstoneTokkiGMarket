...
//mongoose.connect(process.env.MONGO_DB, function(err, db){
    mongoose.connect("mongodb+srv://jinho:456798ab@cluster0.twnvx.mongodb.net/TokkiGmarket?retryWrites=true&w=majority", function(err, db){
       ...
       // 처음 DB연결했을때 채팅방에 이용할 chatroom 모두 가져옴.
        mongoose.connection.db.listCollections({name: /^chats/}, {nameOnly:true}).toArray(function (err, names) {  
        // chats로 시작하는거 다 가져옴. filter은 정규식name은 only option사용
        allchatroomsarray = names;
        for(var i in names) { // 객체 가져오면 name즉 채팅방이름만 collection에서 가져옴 ex chats/bubble2jh,bubble3jh
        ...
        console.log('MongoDB connected...');
                ...
                socket.on('possiblerooms', (possiblerooms) => {
                    ...
                    // 교집합 해버린다. 
                    let finalroom = possiblerooms.filter(x => allchatroomsarray.includes(x)); // 결과 2, 3
                ...
                console.log('서버에서 원래 룸네임-> ' + roomname + '에서 '+ roomToJoin + '룸으로  실행됐다.');
                ...
                socket.on('input', function(data){  // 매개변수 roomname 데이터에 추가했음.
                    let name = data.name;
                    ...
                    // } else {
                        ...
                        chat.insert({name: name, message: message}, function(){ // name이랑, message를 db에 insert하는거다! 100%
